 ...
 *
 * This file is part of Vescafe project.
 *
 ...
 * Created by luca at 10:16:26 AM
 */
package schedulers;
...
public class AppStart extends Job {
	...
	public void doJob() {
		...
		// 3362 - 5 names and pos - 7 - 8
//		GrammarService.instance().fixAppendixAndDeleteOther();
		// 3362 - 2
//		GrammarService.instance().fixNames();
		for (Grammar g : gl) {
			// GrammarService.instance().removePart(g);
...
//			GrammarService.instance().fixTense(g);
		...
		// 3362 - 1
//			GrammarService.instance().fixGrammar(g);
		...
		/*
		 * for(Grammar g : gl) { GrammarService.instance().fixGrammarParts(g); }
		 * for(Grammar g : gl) { GrammarService.instance().fixGrammarParts1(g); }
		 * for(Grammar g : gl) { GrammarService.instance().fixIntonation(g);
		 * GrammarService.instance().fixTense(g); }
		 */
		// 3362 - 6 - 9

//		GrammarService.instance().fixNameCompleteList();
//		GrammarService.instance().fixPosGrammaticalTerms();

		// GrammarService.instance().fixNameCompleteList();
		// GrammarService.instance().fixPosGrammaticalTerms();
		GrammarService.instance().fixNamesAbbreviations();
		boolean found = false;
		for (Grammar g : gl) {
...
//		}
	...
	private void setAuthsToRole(EsAccountManager esdao, List<EsAuth> adminAuths, String role, boolean b) {
		...
		for (EsAuth adminAuth : adminAuths) {
			if (esdao.findAuthByRole(admin.getName()) == null
					|| !esdao.findAuthByRole(admin.getName()).contains(adminAuth)) {
				esdao.addRoleAuths(adminAuth, admin);
				if (b)
					Logger.info("AppStart - setAuthsToRole adding addRoleAuths" + adminAuth.getName() + " "
							+ admin.getName());
			}
			if (esdao.findRoleByAuth(adminAuth.getName()) == null
					|| !esdao.findRoleByAuth(adminAuth.getName()).contains(admin)) {
				esdao.addAuthRoles(adminAuth, admin);
				if (b)
					Logger.info("AppStart - setAuthsToRole adding addAuthRoles" + adminAuth.getName() + " "
							+ admin.getName());
			}
	...
	private void createFakeFeatureValues() {
		/*
		 * List<FeatureValue> list = FeatureValue.findAll(); if(list != null &&
		 * !list.isEmpty()) return;
		 */
...
//		fv1.save();
	...
	private String getRandomOption(List<FeatureOption> opts) {
		/*
		 * List<String> keys = new LinkedList<String>(); for(FeatureOption fo : opts) {
		 * keys.add(fo.optionKey); } //keys.addAll(opts.keySet()); if(keys.size() <=0)
		 * return null;
		 */
		int idx = 0;
